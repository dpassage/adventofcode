...
struct Rule {
    ...
    init?(string: String) {
        ...
        guard let matches = regex.match(string) where
...
func applyRule(rule: Rule, input: String) -> [String] {
    ...
    let split = input.componentsSeparatedByString(rule.left)
    var results = [String]()
    ...
    for i in 0..<split.count-1 {
        ...
        let leftPart = leftSplit.joinWithSeparator(rule.left)
        let rightPart = rightSplit.joinWithSeparator(rule.left)

        let result = [leftPart, rightPart].joinWithSeparator(rule.right)
...
func computeStrings(input: String, rules: [Rule]) -> Set<String> {
    let mapresults = rules.map { (rule) -> [String] in
        return applyRule(rule, input: input)
    }
...
for line in TextFile.standardInput().readLines() {
    ...
    if let rule = Rule(string: line) {
        ...
        let result = computeStrings(line, rules: inputRules)
        print(result)
        print(result.count)
    }
}
